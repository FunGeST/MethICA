...
<ul>
<li>file_CGI : CpG island-based features (Island, Shore, Shelf, out of cgi) from UCSC (no liver specific)
...
<p> Script use to generated CpG_feature.Rdata (feature_table_script) are provide in the RUNNING_MethICA_example folder, data use are in the MethICA_data package.

```{r CpGfeature_create_table, eval = FALSE, echo = TRUE}
...
<ul>
<li> boxplot for one components vs one anotation
</ul>
...
<ul>
<li> corrplot for all components vs all anotations (left = univariate, rigth = multivariate): 
</ul>
